...
class AddSimulationPortWorker
{
  func doSomeWork() {
    ...
    typealias FecthNavListDataCompletionHandler = (_ navList: [NavListModel]?,_ error: Error?) -> ()
    func doFetchNavListData(completionHandler: @escaping FecthNavListDataCompletionHandler) {
        let router = Funds.navList
        Alamofire.request(router).responseJSON { response in
            ...
            if let data = response.data {
                do {
                    let NavListModelData = try JSONDecoder().decode([NavListModel]?.self, from: data)
//                    print("getOrderModelData >> ", NavListModelData.count)
                    completionHandler(NavListModelData, nil)
